 ...
 *
 * \obbz\yii2\widgets\Excel::widget([
 * 		'models' => $allModels,
 ...
 * ]);
 *
 * \obbz\yii2\widgets\Excel::export([
 * 		'models' => $allModels,
 * 		'columns' => ['column1','column2','column3'],
 * 		//without header working, because the header will be get label from attribute label.
 * 		'header' => ['column1' => 'Header Column 1','column2' => 'Header Column 2', 'column3' => 'Header Column 3'],
 ...
 * ~~~
 *
 * $data = \obbz\yii2\widgets\Excel::import($fileName, $config); // $config is an optional
 *
 * $data = \obbz\yii2\widgets\Excel::widget([
 * 		'mode' => 'import',
 * 		'fileName' => $fileName,
 ...
 * ]);
 *
 * $data = \obbz\yii2\widgets\Excel::import($fileName, [
 * 		'setFirstRecordAsKeys' => true, // if you want to set the keys of record column with first record, if it not set, the header with use the alphabet column on excel.
 * 		'setIndexSheetByName' => true, // set this if your excel data with multiple worksheet, the index of array will be set with the sheet name. If this not set, the index will use numeric.
 * 		'getOnlySheet' => 'sheet1', // you can set this property if you want to get the specified sheet from the excel data with multiple worksheet.
 * 	]);
 ...
 * ]);
 *
 * $data = \obbz\yii2\widgets\Excel::import([
 * 			'file1' => $fileName1,
 * 			'file2' => $fileName2,
 * 			'file3' => $fileName3,
 * 		], [
...
class Excel extends \yii\base\Widget {
    ...
    /**
     * @var array style for all excel without header style.
     */
    public $style;

    /**
     * @var array of default header style
     */
    ...
    /**
     * @param null $activeSheet PHPExcel_Worksheet
     * @param null $col
     * @param null $row
     * @param array $style
     */
    public function executeStyle(&$activeSheet = null, $col = null, $row = null, $style = []) {
        ...
        if ($col == null && $row == null) {
            $activeStyle = $activeSheet->getDefaultStyle();
        } else {
            $activeStyle = $activeSheet->getStyle($col . $row);
        }
        foreach ($style as $key => $value) {
            switch ($key) {
                case 'font-weight':
                    if($value == 'bold'){
                        $activeStyle->getFont()->setBold(true);
                    }else{
                        $activeStyle->getFont()->setBold(false);
                    }
                    break;
                case 'background-color':
                    $rgb = str_replace('#', '', $value);
                    $activeStyle->applyFromArray(
                        array(
                            'fill' => array(
                ...
                case 'font-size':
                    ...
                    break;
                case 'apply-from-array':
                    $activeStyle->applyFromArray($value);
                    break;
                case 'validation-list':
                    $objValidation = $activeSheet->getCell($col.$row)->getDataValidation();
                    $data = $value['data'];
                    $options = array_merge([
                        'allowBlank' => true,
                    ...
                    $dataString = \obbz\yii2\utils\ArrayHelper::implodeByKey(',', $data);
                    $objValidation->setFormula1('"'.$dataString.'"');

                    break;
                case 'align':
                    $activeStyle->getAlignment()->setHorizontal($value);
                    break;
                case 'v-align':
                ...
                case 'overflow':
                    if($value == 'wrap'){
                        $activeStyle->getAlignment()->setWrapText(true);
                    }
                    break;
                case 'height':
    ...
    public function executeColumns(&$activeSheet = null, $models, $columns = [], $headers = []) {
        ...
        if ($this->style) {
            $this->executeStyle($activeSheet, null, null, $this->style);
        }
        $hasHeader = false;
        $row = 1;
        $char = 26;
        if ($this->headerTitle) {
            if (isset($this->headerTitle['style'])) {
                $this->executeStyle($activeSheet, null, $row, $this->headerTitle['style']);
            }

            $activeSheet->setCellValue(chr(65) . $row, $this->headerTitle['title']);
            ...
            $col = '';
            if ($num > $char) {
                $col .= chr(64 + floor($num / $char));
            }
            $col .= chr(64 + $num % $char);
            $activeSheet->mergeCells(chr(65) . $row . ':' . $col . $row);
            $row++;
        }
        foreach ($models as $model) {
            ...
            if ($this->setFirstTitle && !$hasHeader) {
                ...
                foreach ($columns as $key => $column) {
                    ...
                    if (isset($this->firstTitle)) {
                        $this->executeStyle($activeSheet, $col, $row, $this->firstTitle);
                    }
                    if (isset($column['headerStyle'])) {
                        $this->executeStyle($activeSheet, $col, $row, $column['headerStyle']);
                    }else if(isset($this->defaultHeaderStyle)){
                        $this->executeStyle($activeSheet, $col, $row, $this->defaultHeaderStyle);
                    }
...
//                    }
    ...
    public function setHeaders() {
        if (isset($this->format) && $this->format == 'Excel5') {
...
// If you're serving to IE over SSL, then the following may be needed
     ...
     * ~~~
     *
     * $data = \obbz\yii2\widgets\Excel::import($fileName, ['setFirstRecordAsKeys' => true]);
     *
     * ~~~
     *
